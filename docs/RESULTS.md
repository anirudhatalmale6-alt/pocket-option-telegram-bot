...
Real EUR/USD history, four timeframes, ten strategies, one-candle expiry.
...
**0 of 40 combinations clear break-even at an 80% payout. Exactly one does at
...
changing sign, and it is the reason one green row in forty is not a finding.
Twelve rows are outright losing. The rest are "unproven": the data cannot tell
them apart from a coin toss.
